...
## 3.4 Normality Test on KDE Values

Let's see is the distribution of educational and healthcare facilities are similar accross Colombo district.
...
Since the p-value of the Shapiro-Wilk test is 2.131641e-85, which is significantly less than 0.05, null hypothesis of normality distribution can be rejected. This indicates that the distribution of educational facility intensity across Colombo significantly deviates from normality, suggesting a non-normal distribution. 
...
Also the healthcare KDE values have p-value of p = 8.578645e-88, which is significantly less than 0.05, which says that the healthcare facility intensity distribution across Colombo also significantly deviates from normality, indicating a non-normal distribution. 
...
- **H₀:** There is no significant correlation between the spatial distribution of educational and healthcare facilities
- **H₁:** There is a significant correlation between the spatial distribution of educational and healthcare facilities

### 3.5.1 Pixel-level KDE Correlation
```{r spearman_kde}
#| code-fold: true
...
cat("Spearman Rank Correlation (Pixel-level KDE)\n")
cat("rho =", round(spearman_kde$estimate, 4), "| p-value =", spearman_kde$p.value, "\n")
```

### 3.5.2 GND-level Density Correlation
```{r spearman_gnd}
#| code-fold: true
#| warning: false

# Calculate facility density per km² for each GND
gnd_density <- cmb_gnds %>%
  ...
  filter(education_density > 0 | healthcare_density > 0) %>%
...
cat("Spearman Rank Correlation (GND-level Density)\n")
cat("rho =", round(spearman_gnd$estimate, 4), "| p-value =", spearman_gnd$p.value, "\n")
```
...
correlation_summary <- data.frame(
  Scale      = c("Pixel level KDE", "GND level Density"),
  Rho        = round(c(spearman_kde$estimate, spearman_gnd$estimate), 4),
  P_value    = c("< 0.001", round(spearman_gnd$p.value, 3)),
  Strength   = c("Moderate positive", "Negligible negative")
)

kable(correlation_summary,
      caption    = "Spearman Rank Correlation: Education vs Healthcare Facility Distribution",
      col.names  = c("Scale", "rho", "p-value", "Strength")) %>%
